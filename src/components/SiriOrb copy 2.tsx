...
// ═══════════════════════════════════════════════════════════════════════════════
const GEMINI_KEY =
  process.env.VITE_GEMINI_API_KEY || "AIzaSyDrwkfrXSYXn86R-h-QKNQAtkH4v7C8A_Y";

const GEMINI_CHAT_MODEL = "gemini-2.5-flash";
const GEMINI_TTS_MODEL = "gemini-2.5-flash-preview-tts";

// Voice: Orus = Firm & confident — perfect for Amit's declarative persona.
// Other good options: Sulafat (Warm), Achird (Friendly), Sadaltager (Knowledgeable)
const TTS_VOICE_NAME = "Orus";

// Gemini TTS outputs raw PCM: 24 kHz, 16-bit signed little-endian, mono
const TTS_SAMPLE_RATE = 24000;
const TTS_CHANNELS = 1;

// ═══════════════════════════════════════════════════════════════════════════════
//  GEMINI CLIENT — singleton via @google/genai
// ═══════════════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════════════
//  GEMINI TTS ENGINE
//  Uses @google/genai to call gemini-2.5-flash-preview-tts.
//  The response is raw PCM (Int16, 24kHz, mono) encoded as base64.
//  We decode it and play via the Web Audio API — no browser TTS, no <audio> tags.
// ═══════════════════════════════════════════════════════════════════════════════

/**
 * Clean text before sending to TTS so the voice
 * doesn't read out markdown chars or raw URLs.
 */
const toSpeakableText = (text: string): string =>
  text
    .replace(/https?:\/\/\S+/g, "")
    .replace(/[*_`#>|]/g, "")
    .replace(/—/g, ", ")
...
/**
 * Director-style TTS prompt — Gemini TTS reads these instructions
 * to shape HOW the text is delivered, not just what is said.
 */
const buildTTSPrompt = (text: string): string =>
  `
# AUDIO PROFILE: Aura
## Voice of Amit Chakraborty

## THE SCENE
A sleek virtual studio. Aura speaks one-on-one with a recruiter, founder,
or engineer visiting Amit's portfolio site. Direct, intimate, professional.

### DIRECTOR'S NOTES
Style: Confident and declarative. Founder energy. Each sentence lands with
weight. No filler. The calm certainty of someone who has shipped 18 apps
and led 21 engineers. Warm undertone beneath the precision.

Pacing: Measured, not slow. Deliberate micro-pauses after key facts.
Commas breathe. Periods land firmly.

Accent: Clear, neutral English with a subtle Indian-educated inflection.
...
/** Raw PCM Int16 bytes (little-endian) → Float32Array for Web Audio */
const pcmInt16ToFloat32 = (bytes: Uint8Array): Float32Array => {
  const samples = Math.floor(bytes.length / 2);
  const f32 = new Float32Array(samples);
  const view = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength);
  for (let i = 0; i < samples; i++) {
    f32[i] = view.getInt16(i * 2, true /* little-endian */) / 32768.0;
  }
  return f32;
};

/** Singleton AudioContext — browsers suspend until user gesture */
...
/**
 * GeminiTTSPlayer
 * Manages one audio playback at a time.
 * Generates audio via @google/genai, decodes PCM, plays via Web Audio API.
 */
class GeminiTTSPlayer {
  ...
  async speak(
    ...
    try {
      ...
      // ── Call Gemini TTS via @google/genai ─────────────────────────────────
      ...
      // ── Extract base64 PCM from response ──────────────────────────────────
      const inlineData =
        response.candidates?.[0]?.content?.parts?.[0]?.inlineData;
      if (!inlineData?.data) throw new Error("Gemini TTS: no audio data");

      // ── Decode PCM → Float32 → AudioBuffer ───────────────────────────────
      const pcmBytes = base64ToUint8Array(inlineData.data);
      const f32Samples = pcmInt16ToFloat32(pcmBytes);
      ...
      audioBuf.copyToChannel(f32Samples, 0);
      ...
      console.warn("GeminiTTS error, falling back to browser TTS:", err);
      onEnd?.();
      await this._browserFallback(text, onStart, onEnd);
    }
  }

  /** Browser speechSynthesis as last-resort fallback */
...
// ═══════════════════════════════════════════════════════════════════════════════
//  AI CHAT — Supabase Edge Function with @google/genai direct fallback
// ═══════════════════════════════════════════════════════════════════════════════
interface GeminiMsg {
  role: "user" | "model";
  parts: { text: string }[];
...
const callGeminiDirect = async (
  ...
  console.log("🟡 Direct Gemini chat fallback...");
  let visitorCtx = "";
  if (user?.name) {
  ...
  try {
    ...
    console.error("Direct Gemini chat error:", err);
    return "";
  }
};
...
const askAura = async (
  ...
  while (attempt <= MAX_RETRIES) {
    if (attempt > 0) setError?.("Waking up AURA...");
  ...
  setError?.(`Issue: ${lastErr.slice(0, 20)}`);
...
const localFallback = (msg: string): string => {
  ...
  if (q.match(/hire|why/))
    return `8 years. 18 apps. 50 thousand real users. HIPAA AI pipelines from nothing. Mobile, AI, and Web3. Led 21 engineers. Reach him at ${PROFILE.email}.`;
  ...
  if (q.match(/web3|defi|nft/))
    return `DeFi11, 100 percent on-chain. Smart contract prize pools. NFT marketplace on Ethereum. Vulcan Eleven, 50 thousand users, Binance Pay. Amit built both.`;
...
const detectQueryIntent = (
  ...
  if (q.match(/\b(contact|email|reach|linkedin|github|connect)\b/))
    return {
      local: true,
      answer: `${PROFILE.email}. LinkedIn: linkedin.com/in/devamitch. GitHub: github.com/devamitch. Open for VP Engineering, CTO, and Principal Architect roles.`,
    };
  ...
  >
    ...
    <div>
      ...
      >
        ...
        <br />
        If Aura was useful, Amit would appreciate your support.
      </div>
    </div>
    <div
      style={{
...
export const SiriOrbNew = () => {
  ...
  // ── TTS via @google/genai + Web Audio API ─────────────────────────────────
  ...
  const open = useCallback(() => {
    ...
    springTo(1, async () => {
      ...
      convoId.current = `c_${Date.now()}_${Math.random().toString(36).slice(2, 7)}`;
      sessionCountedRef.current = false;

      setTimeout(async () => {
        const cs = getStore();
        const profile = cs.userProfile;
        let greeting: string;
        if (profile?.name) {
          const prevTopics = extractInterests(
            cs.getRecentConversationMessages(6),
          );
          const sessionNum = profile.sessionCount || 1;
          greeting = prevTopics.length
            ? `${profile.name}. Session ${sessionNum}. Last time: ${prevTopics.slice(0, 2).join(" and ")}. What do you need today?`
            : `${profile.name}. Session ${sessionNum}. What can I help you with today?`;
        } else {
          greeting = `I'm Aura, Amit Chakraborty's AI. What's your name?`;
          setOnboardStep("ask_name");
        }
        cs.setMessages([{ role: "ai", text: greeting, ts: Date.now() }]);
        speak(greeting);
      }, 400);
    });
  }, [springTo, speak, getStore]);
  ...
  const sendMessage = useCallback(
    async (text: string) => {
      ...
      if (local && answer) {
        s.addMessage({ role: "user", text: msg, ts: Date.now() });
        setTimeout(() => {
          getStore().addMessage({ role: "ai", text: answer, ts: Date.now() });
          speak(answer);
        }, 100);
        return;
      }
      ...
      try {
        ...
        console.error("askAura failed:", err);
      ...
      >
        ...
        >
          ...
          {!isOrb && (
            ...
            >
              {limitHit && isChat && !sessionCountedRef.current && (

...
const toNaturalSpeech = (text: string): string =>
  text
    ...
    .replace(/https?:\/\/\S+/g, "")
    .replace(/[*_`#]/g, "")
    .replace(/\. /g, ".  ") // extra pause after sentences
    .trim();
...
const localFallback = (msg: string): string => {
  ...
  if (q.match(/who|about|introduce/))
    return `Amit Chakraborty. Principal Mobile Architect. 8 years. 18 apps. 50 thousand plus users. Built a health tech engine from scratch, no external libs, twenty one person team. Based in Kolkata.`;
...
const detectQueryIntent = (
  ...
  if (q.match(/\b(contact|email|reach|linkedin|github|connect)\b/))
    return {
      local: true,
      answer: `${PROFILE.email}. LinkedIn: linkedin.com/in/devamitch. GitHub: github.com/devamitch. Open for V P Engineering, C T O, and Principal Architect roles.`,
    };
...
// ═══════════════════════════════════════════════════════════════════════════════
//  VOICE — natural sounding, not robotic
// ═══════════════════════════════════════════════════════════════════════════════
...
const getPreferredVoice = async (): Promise<SpeechSynthesisVoice | null> => {
  ...
  // Priority: natural sounding male English voices
  const priority = [
...
export const SiriOrbNew = () => {
  ...
  // FIX: use getState() for non-reactive reads inside callbacks
  ...
  // Track if this session has been counted for daily limit
  ...
  // FIX: derive limitHit reactively from store, not just at init
  ...
  const speak = useCallback(async (rawText: string) => {
    ...
    // FIX: Natural human voice settings — NOT robotic
    u.rate = 0.9; // slight slow for clarity, not dragging
    u.pitch = 0.95; // near-natural, was 0.52 (way too low/robotic)
    u.volume = 1.0;
  ...
  const open = useCallback(() => {
    ...
    // FIX: Check limit BEFORE opening (don't increment here — increment on first message)
    const s = getStore();
    if (!s.canChat() && !sessionCountedRef.current) {
      // Already at limit, open anyway to show limit screen
    }
    ...
    springTo(1, async () => {
      ...
      // Reset convo ID for this new session
      ...
      setTimeout(async () => {
        ...
        let greeting: string;
        const profile = currentStore.userProfile;

        if (profile?.name) {
          // Build context from previous conversations
          const prevMsgs = currentStore.getRecentConversationMessages(6);
          const prevTopics = extractInterests(prevMsgs);
          const sessionNum = profile.sessionCount || 1;

          if (prevTopics.length) {
            greeting = `${profile.name}. Session ${sessionNum}. Last time: ${prevTopics.slice(0, 2).join(" and ")}. What do you need today?`;
          } else {
            greeting = `${profile.name}. Session ${sessionNum}. What can I help you with today?`;
          }
        } else {
        ...
        currentStore.setMessages([
          { role: "ai", text: greeting, ts: Date.now() },
        ]);
        speak(greeting);
      }, 400);
    });
  ...
  const sendMessage = useCallback(
    async (text: string) => {
      ...
      // FIX: Count the conversation on FIRST message sent, not on open
      if (!sessionCountedRef.current) {
        if (!s.canChat()) {
          // Limit hit — don't process
          return;
        }
        s.incrementDaily();
      ...
      // ONBOARDING FLOW
      if (isOnboarding) {
        // FIX: capture history BEFORE addMessage
        ...
        if (step === "ask_name") {
          ...
          if (company) {
            ...
            reply = `Which company or organization are you with?`;
          }
      ...
      // FIX: Capture history BEFORE adding current user message
      // This prevents double-sending the user message to the AI
      ...
      >
        ...
        >
          ...
          {!isOrb && (
            ...
            >
              {/* FIX: Show limit screen based on reactive `limitHit` */}

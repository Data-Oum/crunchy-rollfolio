  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      if (headingRef.current) {
        ...
        gsap.fromTo(els, { y: 50, opacity: 0, skewY: 2 }, {
      ...
      if (timelineRef.current) {
        ...
        items.forEach((el: Element, i: number) => {
          gsap.fromTo(el, { x: i % 2 === 0 ? -100 : 100, opacity: 0, scale: 0.85 }, {
      ...
      if (skillCards?.length) {
        gsap.fromTo(skillCards, { y: 60, opacity: 0, scale: 0.8 }, {
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div ref={headingRef} className="mb-20">
          ...
          <h2 className="reveal-el text-6xl sm:text-7xl font-black leading-none"
            ...
            <span className="text-gradient-crunchy italic">ARC</span>
          </h2>
            ...
            return (
              <div key={item.yr}
                ...
                <div className="absolute left-5 sm:left-1/2 -translate-x-1/2 z-10 mt-1.5">
                  <motion.div whileInView={{ scale: [0, 1.5, 1] }} viewport={{ once: true }}
                    transition={{ duration: 0.6 }}
                    className="w-4 h-4 rounded-full border-2 border-background"
                    style={{ background: item.color, boxShadow: `0 0 18px ${item.color}` }} />
                </div>
          ...
          <div className="flex items-center gap-4 mb-12">
            ...
            <div>
              ...
              <h2 className="text-5xl sm:text-6xl font-black text-foreground"
                style={{ fontFamily: "'Impact','Arial Black',sans-serif" }}>
                POWER <span className="text-gradient-crunchy italic">LEVELS</span>
              </h2>
            </div>
          </div>
